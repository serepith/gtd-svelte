<script lang="ts">
	...
	$effect(() => {
		...
		if (
			data.nodes.length !== lastDataNodesLength ||
		...
		if (siblings.length > 0) {
			...
			for (let i = 0; i < halfCount; i++) {
				if (siblings[i]) {
					...
					siblings[i].y =
						focusNode.y -
						((halfCount - 1) * siblingVerticalSpacing) / 2 +
						i * siblingVerticalSpacing;
				}
			}
			...
			for (let i = halfCount; i < siblings.length; i++) {
				if (siblings[i]) {
					...
					siblings[i].y =
						focusNode.y -
						((siblings.length - halfCount - 1) * siblingVerticalSpacing) / 2 +
						(i - halfCount) * siblingVerticalSpacing;
				}
			}
		}
	...
	function draw() {
		...
		nodes.forEach((node) => {
			...
			if (isCenterNode) {
				...
				context.fillText(
					'Hierarchical Layout • Click nodes to change focus',
					contentX,
				...
				const maxLength = 15;
				const displayName =
					node.name.length > maxLength ? node.name.substring(0, maxLength) + '...' : node.name;
...
<div class="relative h-full w-full rounded-lg border bg-white">
	<canvas bind:this={canvas} {width} {height} class="h-full w-full cursor-pointer bg-transparent"
	></canvas>
	...
	{#if isEditing && centerNode && centerNode.x !== undefined && centerNode.y !== undefined}
		...
		<div
			class="absolute z-10 rounded-lg border-2 border-amber-400 bg-white p-4 shadow-lg"
			style="left: {overlayX}px; top: {overlayY}px; width: {nodeWidth}px;"
		>
			<div class="mb-3">
				<label class="mb-1 block text-sm font-medium text-gray-700"> Node Name </label>
				<input
					type="text"
			...
			<div class="flex justify-end gap-2">
				<button
					class="rounded bg-gray-200 px-3 py-1 text-sm text-gray-700 transition-colors hover:bg-gray-300"
					on:click={cancelEdit}
				>
					Cancel
				</button>
