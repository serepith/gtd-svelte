<script lang="ts">
	...
	function updateGraph() {
		...
		if (simulation) {
			...
			simulation.force(
				'link',
				d3Force.forceLink(links).id((d) => (d as Node).id)
			);
		...
		nodes.forEach((node) => {
			...
			if (isCenterNode) {
				...
				if (node.x !== undefined && node.y !== undefined) {
					...
					context.fillText(
						'Click to edit • Future: inline editing controls',
						contentX,
...
<div class="h-full w-full rounded-lg border bg-white">
	<canvas bind:this={canvas} {width} {height} class="h-full w-full cursor-pointer"></canvas>
</div>
