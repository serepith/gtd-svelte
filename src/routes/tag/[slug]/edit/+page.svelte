<script lang="ts">
	...
	let graphNodes: Array<{id: string, name: string, x: number, y: number, level: number, isCurrentTag: boolean}> = $state([]);
	let graphEdges: Array<{source: string, target: string}> = $state([]);
	let containerDimensions = $state({ width: 0, height: 0, aspectRatio: 1 });
	
	let taggedTasksPromise: Promise<Task[]> = $state((getTagId(tagName)).then(tag => getTasksInTag(tag?.id || '')));
	...
	function generateMockHierarchy() {
		...
		const edges = allEdges.filter(edge => 
			edge.source === tagName || edge.target === tagName
		);
		...
		const nodes: Array<{id: string, name: string, x: number, y: number, level: number, isCurrentTag: boolean}> = [];
...
<section class="p-4">
	<div class="max-w-4xl mx-auto">
		...
		<!-- Tag Hierarchy Graph -->
		<div class="hierarchy-panel bg-base-100 rounded-box shadow-md mb-6 p-6">
			...
			<div class="graph-container bg-base-200 rounded-lg p-4 relative h-96" bind:this={graphContainer}>
			...
			{#each graphNodes as node}
				...
				>
					<div class="relative">
						...
						{#if !node.isCurrentTag}
							<button 
								class="absolute -top-2 -right-2 w-4 h-4 bg-error hover:bg-error/80 text-error-content rounded-full flex items-center justify-center text-xs font-bold transition-all duration-200"
								onclick={() => removeRelatedTag(node.id)}
		...
		<!-- Tag Properties -->
		<div class="properties-panel bg-base-100 rounded-box shadow-md mb-6 p-6">
			<h2 class="text-xl font-semibold mb-4">Tag Properties</h2>
		...
		<div class="tasks-panel bg-base-200 rounded-box shadow-md">
			<div class="p-4 border-b border-base-300">
				...
				<p class="text-sm text-base-content/70 mt-1">These tasks will be affected by your changes</p>
			</div>
			
			{#await taggedTasksPromise}
				<div class="p-8 text-center">
					<div class="loading loading-spinner loading-lg mx-auto"></div>
					<p class="mt-4 text-base-content/70">Loading tasks...</p>
				</div>
			{:then taggedTasks}
				{#if taggedTasks.length === 0}
					<div class="p-8 text-center">
						<p class="text-lg text-base-content/70">
							No tasks found with tag <span class="tag-chip inline-block mx-1">#{tagName}</span>
						</p>
					</div>
				{:else}
					<div class="p-4">
						{#each taggedTasks as task (task.id)}
							<div class="task-row flex items-center justify-between py-3 px-2 border-b border-base-300/50 last:border-b-0">
								<div class="task-content">
									<div class="task-text" class:line-through={task.completed}>{task.name}</div>
								</div>
								<div class="task-date text-sm text-base-content/70">
...
<style>
	...
	.hierarchy-panel, .properties-panel, .tasks-panel {
		transition: all 0.2s ease;
	}
