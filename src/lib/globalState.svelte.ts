...
import { CACHE_SIZE_UNLIMITED, collection, CollectionReference, getFirestore, onSnapshot, persistentLocalCache, persistentMultipleTabManager, QueryDocumentSnapshot, Timestamp, type DocumentData, type Firestore, type FirestoreDataConverter, type SnapshotOptions } from "firebase/firestore";
...
let nodesCollection = $derived(firebase.user && firebase.db ? collection(firebase.db, 'users', firebase.user?.uid, 'nodes') : null);
...
export { firebase, items, getNodesCollection };
...
export function initFirebase() {
  ...
  if(firebase.auth) {
    firebase.auth.onAuthStateChanged((currentUser) => {
      ...
      if(nodesCollection) {
        unsubscribe = onSnapshot(nodesCollection.withConverter(taskConverter), (snapshot) => {
